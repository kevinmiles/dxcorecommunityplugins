    ...
    internal class SA1019_MemberAccessSymbolsMustBeSpacedCorrectly : StyleCopRule
    {
        ...
        public SA1019_MemberAccessSymbolsMustBeSpacedCorrectly()
            : base(new AggregatedIssueLocator(new ICodeIssueLocator[]
                {
                    new AllTokensPrecededByBannedElementIssueLocator(ElementTokens, (token, violation) => token.CsTokenType == CsTokenType.OperatorSymbol && memberAccessSymbols.Contains(token.Text), CsTokenType.WhiteSpace),
                    new AllTokensPrecededByBannedElementIssueLocator(AttributesTokens, (token, violation) => token.CsTokenType == CsTokenType.OperatorSymbol && memberAccessSymbols.Contains(token.Text), CsTokenType.WhiteSpace),
