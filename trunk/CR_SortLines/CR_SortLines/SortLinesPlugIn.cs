...
namespace CR_SortLines {
	...
	public class SortLinesPlugIn: StandardPlugIn {
		...
		private void actionSortLines_Execute(DevExpress.CodeRush.Core.ExecuteEventArgs ea) {
			...
			Log.Enter(ImageType.Info, "{0}Sorting lines.", LOG_PREFIX);
			...
			try{
				...
				if(CodeRush.Selection.Exists){
					...
					selectFinalSet = true;
					firstLine = CodeRush.Selection.Active.TopEditLine;
					lastLine = CodeRush.Selection.Active.BottomEditLine;
					// If we've selected to the start of the next line, we don't actually
					// want to sort that next line, so subtract 1.
					if(CodeRush.Selection.Active.BottomPoint.AtStartOfLine){
						lastLine--;
					}
				}
				else{
				...
				if(selectFinalSet){
					...
					CodeRush.Documents.ActiveTextDocument.SelectText(firstLine, 1, newLastLine, CodeRush.Documents.ActiveTextDocument.LengthOfLine(newLastLine) + 1);
				}
				else{
